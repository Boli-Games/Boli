...
// party/server.ts
var MAX_PLAYERS = 8;
var BoliRoom = class extends Server {
  static {
    __name(this, "BoliRoom");
  }
  hostId = null;
  playing = false;
  ...
  onConnect(conn) {
    if (this.playing) {
      send(conn, { t: "error", message: "La ronda ya empez\xF3." });
      conn.close();
      return;
    }
  ...
  onClose(conn) {
    ...
    if (conn.id === this.hostId) {
      this.emit({ t: "closed", reason: "El anfitri\xF3n se fue." });
...
var server_default = {
  async fetch(request, env2) {
    return await routePartykitRequest(request, env2) ?? new Response("boli rooms", { status: 200 });
  }
};
...
// .wrangler/tmp/bundle-KrG212/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KrG212/middleware-loader.entry.ts
