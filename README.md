...
`time set` usa hora del mundo. Referencias útiles: `00:00` noche, `06:00` amanecer, `12:00` mediodía, `18:00` atardecer.
...
- **TypeScript** + **Vite**
- **Three.js** (mundo y cielo)
- **PartyKit / PartyServer** + **Cloudflare Workers** (salas en tiempo real, con Wrangler)
...
`party:dev` levanta las salas en `127.0.0.1:8787`. `dev` abre el cliente en Vite (puerto `5173`).

En producción el cliente usa `VITE_PARTYKIT_HOST`.
